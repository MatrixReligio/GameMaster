...
- **Metal 原生的 DirectX 轉換**——DirectX 11/12 遊戲依執行環境經由 Apple 的
  D3DMetal（Game Porting Toolkit）或開源的 DXMT 執行，均為 Metal 原生效能。
- **貼合你的 Mac**——新遊戲瓶一開始就依你的顯示器給出匹配的畫面預設值（算繪解析度、
  MetalFX 放大與倍數），並提供「為這台 Mac 推薦」按鈕可隨時重新套用。
- **預設簡單，需要時強大**——一開始就有合理的預設值；Retina、同步模式、MetalFX（含放大
  品質與幀率上限）、光線追蹤，以及每個遊戲瓶的環境變數，都只在一個展開三角形之後。
...
GameMaster 不內建任何執行環境——它們會在首次執行時下載，且都固定到精確的 URL 與
SHA-256 摘要：

- **預設執行環境**是社群維護的 *Game Porting Toolkit* 構建
  （[Gcenx](https://github.com/Gcenx/game-porting-toolkit)），直接從該專案的
  發佈頁下載。依該專案的封裝方式，其中已包含 Apple 的 **D3DMetal** 評估函式庫，
  因此 DirectX 11/12 遊戲開箱即用。
- **Steam 遊戲瓶**執行在一套開源執行環境上，由
  [Sikarugir](https://github.com/Sikarugir-App) 的 CrossOver 衍生 Wine 引擎與
  [DXMT](https://github.com/3Shain/dxmt)（Direct3D 10/11 → Metal，LGPL）組裝
  而成——`scripts/assemble-steam-runtime.sh` 記錄了精確的固定來源。

Apple 的 D3DMetal 函式庫是專有軟體；本儲存庫與 GameMaster 自己的發佈資產從不
重新託管它們。若要更新至 Apple 最新的圖形層，GameMaster 會引導你從
[Apple Developer](https://developer.apple.com/download/all/)
下載免費的 *"Evaluation environment for Windows games"*（只需免費的 Apple ID
即可）並在本地匯入——匯入前會驗證這些函式庫確由 Apple 簽署，且它們始終不離開
你的機器。
...
純邏輯核心位於 Swift 套件（`Sources/GM*`），由大量測試覆蓋；SwiftUI 應用程式外殼
