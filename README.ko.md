...
- **Metal 네이티브 DirectX 변환** — DirectX 11/12 게임은 런타임에 따라 Apple의
  ...
  Metal 네이티브 성능을 냅니다.
- **내 Mac에 맞춤** — 새 보틀은 디스플레이에 맞춘 그래픽 기본값(렌더링 해상도,
  MetalFX 업스케일과 배율)으로 시작하며, "이 Mac에 맞게 추천" 버튼으로 언제든 다시
  적용할 수 있습니다.
- **기본은 단순하게, 필요할 땐 강력하게** — 처음에는 합리적인 기본값으로 시작하고,
  Retina, 동기화 모드, MetalFX(업스케일 품질과 프레임레이트 제한 포함), 레이 트레이싱,
  보틀별 환경 변수는 펼침 삼각형 하나만 열면 바로 사용할 수 있습니다.
