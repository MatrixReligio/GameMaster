...
- **あなたの Mac に最適化** — 新しいボトルはディスプレイに合わせた描画の既定値
  （描画解像度、MetalFX アップスケールと倍率）で始まり、「この Mac におすすめ」ボタンで
  いつでも再適用できます。
- **標準はシンプル、必要なときはパワフル** — 最初は無理のない既定値を用意。Retina、
  同期モード、MetalFX（アップスケール品質とフレームレート上限つき）、レイトレーシング、
  ボトルごとの環境変数は、開閉用の三角形をひとつ開くだけで利用できます。
